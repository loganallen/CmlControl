...
let get_staged_help (idx : index) (cmt_idx : index) : string list =
  let find_staged acc (f,h) =
    let hash = try List.assoc f cmt_idx with Not_found -> "nil" in
    if hash = h then acc else f::acc
  in
  List.(fold_left find_staged [] idx |> sort (Pervasives.compare))
...
let get_staged (idx: index) : string list =
  try
    ...
    Tree.(read_tree "" cmt.tree |> tree_to_index) |> get_staged_help idx
  with
...
let get_changed_as_index (cwd : string list) (idx : index) : index =
  let find_changed acc fn =
    ...
    | Not_found -> acc
  in
  List.(fold_left find_changed [] cwd |> sort (Pervasives.compare))
...
let get_untracked (cwd : string list) (idx : index) : string list =
  ...
  in
  List.(fold_left find_untracked [] cwd |> sort (Pervasives.compare))
...
let get_deleted (cwd_files : string list) (idx : index) : string list =
  ...
  try
    let cmt = Head.get_head_safe () |> Object.parse_commit in
    Tree.(read_tree "" cmt.tree |> tree_to_index) |> files_in_index
